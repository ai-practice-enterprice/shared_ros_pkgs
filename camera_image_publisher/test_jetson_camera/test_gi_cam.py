...
class GstOpenCVConverter():
    def __init__(self,pipeline_source: str,show_opencv_window: bool = False, pipeline_record: str = False, record_fps: float = 30.0):
        Gst.init()
        ...
        self.timestamp = 0
        if pipeline_record:
            self.setup_recording_pipeline(pipeline_record)
    ...
    def push_frame_async(self, buf):
        def _push():
            ret = self.record_sink.emit("push-buffer", buf)
            if ret != Gst.FlowReturn.OK:
                print("Error pushing buffer to recording pipeline")
        Thread(target=_push).start()
    
    def setup_recording_pipeline(self,pipeline_str):
        ...
        self.record_sink = self.record_pipeline.get_by_name("recordsrc")
        # !!! adjust width/height if needed
        caps = Gst.Caps.from_string(f"video/x-raw,format=RGB,width=640,height=480,framerate={int(self.record_fps)}/1") 
        self.record_sink.set_property("caps", caps)
        self.record_sink.set_property("format", Gst.Format.TIME)
        self.recording = True 
...
if __name__ == "__main__":
    ...
    gst_out = "appsrc name=recordsrc is-live=true block=true format=time ! x264enc ! mp4mux ! filesink location=output.mp4"
